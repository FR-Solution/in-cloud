...
### 5.2.3.1.2.1. Переменные окружения PKI
...
### 5.2.3.1.2.2. Рабочая директория для PKI
<CodeBlock language="bash">
    {dedent`
...
### 5.2.3.1.2.4. Приватные ключи PKI
```bash
openssl genrsa \
    -out "${FRONT_PROXY_CA_KEY_PATH}" ${FRONT_PROXY_CA_KEY_SIZE}
```

### 5.2.3.1.2.5. Публичные ключи PKI
```bash
openssl req \
    -x509 \
    -new \
