...
### 5.2.3.2.2.1. Переменные окружения PKI
...
### 5.2.3.2.2.2. Рабочая директория для PKI
<CodeBlock language="bash">
    {dedent`
...
### 5.2.3.2.2.4. Генерация приватного ключа
```bash
openssl genrsa \
    -out "${KUBELET_SERVER_KEY_PATH}" ${KUBELET_SERVER_KEY_SIZE}
```

### 5.2.3.2.2.5. Генерация CSR
```bash
openssl req \
    -new \
