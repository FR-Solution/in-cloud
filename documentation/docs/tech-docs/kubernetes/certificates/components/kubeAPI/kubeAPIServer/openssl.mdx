...
### 5.1.3.2.7.1. Переменные окружения PKI
...
### 5.1.3.2.7.2. Рабочая директория для PKI
<CodeBlock language="bash">
    {dedent`
...
### 5.1.3.2.7.4. Генерация приватного ключа
```bash
openssl genrsa \
    -out "${KUBERNETES_SERVER_KEY_PATH}" ${KUBERNETES_SERVER_KEY_SIZE}
```

### 5.1.3.2.7.5. Генерация CSR
```bash
openssl req \
    -new \
