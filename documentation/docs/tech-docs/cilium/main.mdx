...
В столбцах `ADDR` и `ADDR2` указаны служебные IP адреса выделенные из первого назначенной на ноду подсети из дефолтного пула. Нельзя удалять с ноды подсеть, с указанными адресами - это может привести к ошибкам.

4)Чтобы освободить какую-нибудь подсеть выполните следующие действия:

4.1) Определите ноду с которой хотите удалить подсеть. Например на ноде `bcloud-k8s-sandbox-test-csko-1-default-nf56t-mdtbd-74pgq` две подсети `10.5.120.128/27` `10.5.120.32/27` и служебные адреса `10.5.120.154` `10.5.120.131`.

4.2)Необходимо закордонить ноду:

```bash
export NN=bcloud-k8s-sandbox-test-csko-1-default-nf56t-mdtbd-74pgq
...
4.5)Редактируем выбранную `ciliumnode` `bcloud-k8s-sandbox-test-csko-1-default-nf56t-mdtbd-74pgq`. Сокращаем реквест до значения меньше либо равное `30`, и удаляем строку с выбранной подсетью:
...
4.7)Проверяем, что пул освобожден и если какой-то ноде нехватало адресов, она должна получить новые подсети с маской `/32`.
...
bcloud-k8s-sandbox-test-csko-1-default-nf56t-mdtbd-74pgq   map[cidrs:[10.5.120.128/27 10.5.120.32/32 10.5.120.33/32] pool:default]   10.5.120.154   10.5.120.131
```
