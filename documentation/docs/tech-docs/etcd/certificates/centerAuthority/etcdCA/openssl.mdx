...
### 5.1.3.1.3.1. Переменные окружения PKI
...
### 5.1.3.1.3.2. Рабочая директория для PKI
<CodeBlock language="bash">
    {dedent`
...
### 5.1.3.1.3.4. Приватные ключи PKI
```bash
openssl genrsa \
    -out "${ETCD_CA_KEY_PATH}" ${ETCD_CA_KEY_SIZE}
```

### 5.1.3.1.3.5. Публичные ключи PKI
```bash
openssl req \
    -x509 \
    -new \
