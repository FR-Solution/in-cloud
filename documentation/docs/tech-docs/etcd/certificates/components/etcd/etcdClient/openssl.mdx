...
### ETCD (HC) client certificate
#### Переменные окружения openssl для сертификата `kube-etcd-healthcheck-client`
...
{dedent`
    ...
    export ETCD_CA_CRT_PATH="${CERTIFICATES.kubernetesCA.crtPath}"
    export ETCD_CA_KEY_PATH="${CERTIFICATES.kubernetesCA.keyPath}"
`}
</CodeBlock>
...
#### Генерация приватного ключа для сертификата `kube-etcd-healthcheck-client`
...
#### Генерация CSR для сертификата `kube-etcd-healthcheck-client`
