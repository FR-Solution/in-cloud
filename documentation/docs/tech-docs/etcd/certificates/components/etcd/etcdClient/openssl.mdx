...
### 5.1.3.2.2.1. Переменные окружения PKI
...
### 5.1.3.2.2.2. Рабочая директория для PKI
<CodeBlock language="bash">
    {dedent`
...
### 5.1.3.2.2.4. Генерация приватного ключа
```bash
openssl genrsa \
    -out "${ETCD_CLIENT_KEY_PATH}" ${ETCD_CLIENT_KEY_SIZE}
```

### 5.1.3.2.2.5. Генерация CSR
```bash
openssl req \
    -new \
