...
import { CodeAndInputKectl } from '@site/src/components/commonBlocks'
...
> Нужно указать NAMESPACE и NAME pod-а.

```
export SEARCH_NAMESPACE="kube-system"
export SEARCH_POD_NAME="etcd"
```
...
alias ectl='crictl exec \
  ...
  etcdctl \
  $(certpathssearch ${SEARCH_POD_NAME} ${SEARCH_NAMESPACE})'
```
